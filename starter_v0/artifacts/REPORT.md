...
|---|---|---|---|
| **Phạm Hà Anh** | **Trưởng nhóm** (Leader & Core Engine) | • Thiết kế & Tối ưu System Prompt qua 4 phiên bản (v0 ➔ v3)<br>• Khai báo danh sách & tham số schema của Tools<br>• Phát triển Custom Tool mới 100% `find_paper_code`<br>• Đăng ký & Tích hợp Tool vào Engine chính<br>• Biên soạn 10 Team Eval Cases (5 single + 5 multi)<br>• Cấu hình Gemini Provider & Tự động hóa Benchmark<br>• Phân tích nguyên nhân lỗi & Ghi log phiên bản | • `artifacts/system_prompt.md`<br>• `artifacts/tools.yaml`<br>• `tools/find_paper_code/`<br>• `tools/__init__.py`<br>• `data/eval_group.json`<br>• `providers/gemini_provider.py`<br>• `artifacts/version_log.csv` |
...
|---|---|---|
| `clarify` | Hỏi lại người dùng khi thiếu thông tin (Handle, URL) hoặc xin xác nhận yes/no trước hành động nhạy cảm | Không |
| `timeline` | Lấy các bài đăng Twitter mới nhất của một tài khoản cụ thể theo screenname/handle | Không |
| `social_search` | Tìm kiếm bài đăng trên mạng xã hội theo từ khóa/chủ đề (Top hoặc Latest) | Không |
| `lookup` | Tra cứu thông tin tin tức trên web (General hoặc News) theo khoảng thời gian | Không |
| `fetch` | Đọc trích xuất nội dung văn bản từ một URL cụ thể | Không |
| `format` | Trình bày danh sách dữ liệu thu thập được thành bản tóm tắt markdown | Không |
| `papers` | Tìm kiếm các bài báo khoa học trên arXiv theo từ khóa | Không |
| `paper_text` | Trích xuất nội dung văn bản của một bài báo arXiv | Không |
| `policy` | Tra cứu trong sổ tay quy định nội bộ của công ty | Không |
| `send` | Đăng/gửi bản tin lên Telegram (chỉ chạy sau khi đã được người dùng xác nhận) | Không |
| `find_paper_code` | **[Tool mới nhóm viết]** Tìm kiếm mã nguồn (official/community GitHub implementation) đi kèm bài báo khoa học | **Có (Mới 100%)** |
